...
# Configurações principais da aplicação Bob
MY_PORT = 5001
PEER_PORT = 5000
...
# Primos para gerar as chaves RSA de Bob (devem ser diferentes dos de Alice)
P_BOB = 67 
Q_BOB = 71

# Gera o par de chaves (pública e privada) para Bob
...
def receive_key_route():
    """
    Endpoint para receber a chave pública do peer (Alice).
...
def send_key_to_peer():
    """
    Envia a chave pública de Bob para Alice via HTTP POST.
    Tenta várias vezes caso Alice ainda não esteja disponível.
...
def send_msg_to_peer(message_text):
    ...
    Garante que só o destinatário (Alice) poderá ler a mensagem.
    """
...
def start_chat_interface():
    ...
    # Envia a chave pública para Alice
    ...
    # Aguarda a chave pública de Alice
